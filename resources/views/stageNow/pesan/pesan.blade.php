							...
							<div class="mailbox-read-info">
								...
								<h5>{{ $users->nama }} ({{ $tipe->nama }})
									...
									<h5 style="color: white">You <font style="font-size: 12px;">({{ $users->nama }})</font>
										@endif
										@endforeach
										<span class="mailbox-read-time pull-right" style="color: black">{{ \Carbon\Carbon::createFromTimestamp(strtotime($pesans->created_at))->diffForHumans() }}</span></h5>
									...
									<div class="mailbox-read-message">
										...
										<span class="glyphicon glyphicon-paperclip"></span> <a href="{{ $pesans->url_lampiran }}" target="_blank">{{ $pesans->lampiran }}</a>
										@endif
										{{--<p>Thanks,<br>Jane</p>--}}
									</div>
									@else
